...
export default {
  ...
  methods: {
    ...
    init (BMapOffline) {
      ...
      const map = new BMapOffline.Map($el, {enableHighResolution: this.highResolution, enableMapClick: this.mapClick})
      this.map = map
      ...
      this.$emit('ready', {BMapOffline, map})
      // Debug
      // global.map = map
      // global.mapComponent = this
    },
    getCenterPoint () {
      const {center, BMapOffline} = this
      switch (checkType(center)) {
        case 'String': return center
        case 'Object': return new BMapOffline.Point(center.lng, center.lat)
        default: return new BMapOffline.Point()
      }
    },
    initMap (BMapOffline) {
