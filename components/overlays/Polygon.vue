...
export default {
  ...
  methods: {
    load () {
      const {BMapOffline, map, path, strokeColor, strokeWeight, strokeOpacity, strokeStyle, fillColor, fillOpacity, editing, massClear, clicking} = this
      const overlay = new BMapOffline.Polygon(path.map(item => createPoint(BMapOffline, {lng: item.lng, lat: item.lat})), {
        strokeColor,
        strokeWeight,
        strokeOpacity,
        strokeStyle,
