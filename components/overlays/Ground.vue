...
export default {
  ...
  watch: {
    bounds: {
      handler (val) {
        ...
        this.originInstance.setBounds(createBounds(BMapOffline, val))
      },
      deep: true
    },
    opacity (val) {
  ...
  methods: {
    load () {
      const {BMapOffline, map, bounds, opacity, imageURL, displayOnMinLevel, displayOnMaxLevel} = this
      const overlay = new BMapOffline.GroundOverlay(bounds && createBounds(BMapOffline, bounds), {
        opacity,
        imageURL,
        displayOnMaxLevel,
        displayOnMinLevel
