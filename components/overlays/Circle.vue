...
export default {
  ...
  watch: {
    'center.lng' (val, oldVal) {
      const {BMapOffline, originInstance, isEditing, disableEditing, enableEditing, center, editing} = this
      if (!isEditing) {
        disableEditing()
        const lng = val
        if (val.toString() !== oldVal.toString() && lng >= -180 && lng <= 180) {
          originInstance.setCenter(createPoint(BMapOffline, {lng, lat: center.lat}))
        }
    ...
    'center.lat' (val, oldVal) {
      ...
      if (!isEditing) {
        ...
        if (val.toString() !== oldVal.toString() && lat >= -74 && lat <= 74) {
          originInstance.setCenter(createPoint(BMapOffline, {lng: center.lng, lat}))
        }
        editing && enableEditing()
      }
    },
  ...
  methods: {
    ...
    load () {
      const {BMapOffline, map, center, radius, strokeColor, strokeWeight, strokeOpacity, strokeStyle, fillColor, fillOpacity, editing, massClear, clicking, enableEditing, disableEditing, getEditingKey, editingKey} = this
      const overlay = new BMapOffline.Circle(createPoint(BMapOffline, {lng: center.lng, lat: center.lat}), radius, {
        strokeColor,
        strokeWeight,
        strokeOpacity,
        strokeStyle,
