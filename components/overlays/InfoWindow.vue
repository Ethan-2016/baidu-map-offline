...
export default {
  ...
  methods: {
    ...
    load () {
      const {BMapOffline, map, show, title, width, height, maxWidth, offset, autoPan, closeOnClick, message, maximize, bindObserver, $parent} = this
      const $content = this.$el
      const overlay = new BMapOffline.InfoWindow($content, {
        width,
        height,
        title,
        maxWidth,
    ...
    openInfoWindow () {
      const {BMapOffline, $container, position, originInstance} = this
      $container.openInfoWindow(originInstance, createPoint(BMapOffline, position))
    },
    closeInfoWindow () {
