...
export default {
  ...
  methods: {
    ...
    load () {
      const instance = this
      const {location, selectFirstResult, autoViewport, highlightMode, keyword, search, BMapOffline, map, originInstance} = this
      const _location = location ? isPoint(location) ? createPoint(BMapOffline, location) : location : map
      const route = this.originInstance = new BMapOffline.BusLineSearch(_location, {
        renderOptions: {
          map,
          panel: this.$el,
          selectFirstResult,
