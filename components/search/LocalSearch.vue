...
export default {
  ...
  methods: {
    searchNearby (nearby) {
      const {originInstance, keyword, customData, BMapOffline} = this
      originInstance.searchNearby(keyword, createPoint(BMapOffline, nearby.center), nearby.radius, customData)
    },
    searchInBounds (bounds) {
      const {originInstance, keyword, customData, BMapOffline} = this
      originInstance.searchInBounds(keyword, createBounds(BMapOffline, bounds), customData)
    },
    search () {
    ...
    load () {
      const instance = this
      const {map, BMapOffline, search, pageCapacity, autoViewport, selectFirstResult, highlightMode, location, originInstance} = this
      const _location = location ? isPoint(location) ? createPoint(BMapOffline, location) : location : map
      const route = this.originInstance = new BMapOffline.LocalSearch(_location, {
        onMarkersSet (e) {
