...
export default {
  ...
  methods: {
    load () {
      const {width, BMapOffline, map, $parent} = this
      const parent = this.parent = $parent.originInstance || map
      if (this.originInstance) {
        parent.removeContextMenu(this.originInstance)
      }
      const menu = this.originInstance = new BMapOffline.ContextMenu()
      for (const item of this.$children) {
        if (item.seperator) {
          menu.addSeparator()
          continue
        }
        const menuItem = new BMapOffline.MenuItem(item.text, function (point, pixel) {
          item.callback({
            point,
            pixel,
            BMapOffline,
